...
data %>%
  group_by(Medium) %>%
  summarise(p.value.sw = shapiro.test(Carbon_mgL)$p.value) # only KM is normally distributed

leveneTest(Carbon_mgL ~ Medium, data = data)[3] # variances arent equal, p = 0.03612178

summary <- group_by(data, Medium) %>%
  summarise(count = n(),
...
res_aov <- aov(Carbon_mgL ~ Medium, data = data)
summary(res_aov) # P is significant (0.00147 < 0.05), so the different groups are not equal like assumed
TukeyHSD(res_aov) # significant for KB-ADaM, KM-ADaM, KH-KB
